...
> За основу взято попередні домашні завдання і підхід до документації (структура, приклади, поради щодо destroy) на кшталт README з модулем RDS/Aurora. fileciteturn8file0
...
### ImagePullBackOff
Причина: в ECR нема потрібного тега або `values.yaml` ще не оновився.  
Рішення: перевір Jenkins job, що image запушений, а `values.yaml` має актуальний `image.tag`.

### Два LoadBalancer service у namespace django
Може з’явитись, якщо раніше деплоїла Helm вручну, а потім ArgoCD створив свої ресурси.
Рекомендація: залишити один сервіс (або узгодити імена ресурсів у chart).

### HPA показує `<unknown>/70%`
Зазвичай означає, що в кластері відсутній metrics-server або він не дає метрики. Це не блокує деплой, але впливає на демонстрацію автомасштабування.

---

## 10) Важливо про витрати (AWS cost safety)
